...
6. 進行五種 variation 的消融研究，量化各主要設計決策（calibration、threshold tuning、class balancing、log features）對 recall 與機率品質的貢獻。
...
為驗證最終模型中各項設計決策對篩選目標是否有實質貢獻，本研究進行消融分析：每次只移除一個元件並重新在 test set 上評估。由於此任務以降低漏判 hazardous 樣本為優先，分析以 recall、F1 與 PR-AUC 為主要觀察軸，accuracy 僅作輔助參考而非比較依據。

本研究比較五種設定：

| 版本 | 設定 | 與 Full Model 的差異 |
|---|---|---|
| V0 | Full Model | Balanced Random Forest + sigmoid calibration + 經 validation 調整的 threshold（0.19） |
| V1 | w/o Calibration | 原始 Balanced Random Forest，並對原始分數調整 threshold（0.34） |
| V2 | w/o Threshold Tuning | 校準模型於 default threshold = 0.5 下評估 |
| V3 | w/o Class Balancing | Random Forest 不使用 `class_weight="balanced_subsample"`，其餘與 V0 相同（threshold 0.24） |
| V4 | w/o Log Features | 訓練資料只使用七個非 log 轉換特徵，其餘與 V0 相同（threshold 0.29） |

對應的 test set 指標如下：

| 版本 | 閾值 | Accuracy | Precision | Recall | F1 | PR-AUC | ROC-AUC |
|---|---:|---:|---:|---:|---:|---:|---:|
...
移除 calibration（V1）後 recall 達到所有設定中最高的 0.8198，但同時產生最多的 false positive。Calibration 以稍微降低 recall 為代價換取較佳的機率品質：Brier score 從 0.0673 降至 0.0600，使得模型輸出更適合作為連續的風險分數。移除 threshold tuning（V2）造成最劇烈的變化：recall 下降至 0.2986，漏判 930 個 hazardous 樣本，而 accuracy 上升至 0.9119。此結果再次說明在類別不平衡的篩選任務中，default threshold = 0.5 並不合適，而僅以 accuracy 評估會掩蓋實際篩選效益的下降。

移除 class balancing（V3）使 recall 從 0.7474 下降至 0.5958，多漏判 201 個 hazardous 樣本；Precision 與 Accuracy 雖上升，但這正是篩選目標不應追求的取捨方向。移除 log features（V4）的結果較為細緻：PR-AUC、ROC-AUC 與 F1 略高於 V0，但 recall 從 0.7474 下降至 0.6244，多漏判 163 個 hazardous 樣本。Log 轉換降低了尺寸與距離分布的右偏程度，使模型在低機率區段（邊界 hazardous 樣本集中之處）有更穩定的決策行為。對於 ranking 型指標，log features 並未提供額外效益，但對本研究所追求的 recall 目標仍有助益。

在四個被移除的元件中，有三個（V2、V3、V4）使 test-set recall 低於 Full Model。V1 是唯一移除後 recall 反而高於 V0 的設定（0.8198 vs 0.7474），但代價是機率品質下降：Brier score 從 0.0600 惡化至 0.0673，使模型不再適合作為風險分數。以對 recall 的絕對影響量級排序，四個元件依序為 threshold tuning（Δ = 0.449）、class balancing（Δ = 0.152）、log features（Δ = 0.123）、calibration（Δ = 0.072）。Threshold tuning 是影響最大的設計，而 calibration 的貢獻則應以機率品質而非 recall 來理解。綜合上述證據，最終設計—calibrated Balanced Random Forest 搭配 validation 調整後的 threshold—在 recall 與機率品質之間取得最佳平衡，適合作為風險篩選任務的部署設定。
...
此案例的預測機率 0.1897 僅比 threshold 0.19 低 0.0003，屬於非常接近決策邊界的樣本。SHAP 顯示四個尺寸相關特徵（`est_diameter_min`、`log_est_diameter_mean`、`est_diameter_range`、`est_diameter_mean`）將機率往上推，而 `log_miss_distance` 將機率往下拉；模型在此衝突訊號下做出保守判斷。這種案例在實務上值得額外檢查，因為小幅調整 threshold 即可能改變分類結果；§4.4 的 V2 消融結果也在系統層級展現同樣的閾值敏感性。
...
本研究建立了一個可重現、可解釋的 NEO hazardous 二元分類流程。結果顯示，在高度不平衡資料下，accuracy 不能作為主要評估指標；相較之下，PR-AUC、F1、recall 與 confusion matrix 更能反映模型是否適合風險篩選情境。Balanced Random Forest 在 validation set 上表現最佳，經 sigmoid calibration 與 threshold tuning 後，在 test set 上達到 F1 = 0.5332、recall = 0.7474、precision = 0.4145。五種 variation 的消融研究進一步確認，threshold tuning 是影響最大的單一設計（移除後 ΔRecall = 0.449），其次依序為 class balancing 與 log features；calibration 則以少量 recall 換取顯著提升的機率品質。可解釋性分析顯示，接近距離、星等與尺寸相關特徵是模型主要依據。
