...
在這四個被移除的元件中，沒有任何一個的移除可以保留 Full Model 的 recall 水準。影響量級由大至小依序為 threshold tuning、class balancing、log features、calibration。此結果支持最終設計：採用 calibrated Balanced Random Forest 並使用 validation 調整後的 threshold 作為部署設定。
